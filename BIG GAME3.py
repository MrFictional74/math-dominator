...
class TargetNumber:
    def __init__(self, level):
        ...
        # 40% chance it's a correct multiple
        if random.random() < 0.6:
            ...
            if level == 1:
                # Every number is a multiple of 1, so fudge it
                self.value = random.randint(101, 199)
                self.is_correct = False
            else:
                # Find a number that is NOT a multiple of the level
                attempts = 0
                while True:
                    ...
                    if n % level != 0:
                        ...
                        break
                    attempts += 1
                    if attempts > 100:  # emergency exit
                        self.value = n
                        self.is_correct = False
                        break
...
while running:
        ...
        for target in targets[:]:
            ...
            if laser.rect.colliderect(target_rect):
                if target.is_correct:
                    ...
                    health -= 1
                    print(f"Wrong target! Health: {health}")
    ...
    if p_score >= level * 100:
        level += 1
        print(f"Level up! You're now on level {level}")
